...
const STORAGE_KEY = "drop-share:recent";
 ...
 */
export function addRecentItem(id: string, visitedAt: number = Date.now()): RecentItem[] {
  const items = [{ id, visitedAt }, ...getRecentItems().filter((item) => item.id !== id)];
