  ...
  const startUpload = useCallback(
    async (mode: UploadMode) => {
      ...
      try {
        ...
        window.setTimeout(() => void navigate(result.url), REDIRECT_DELAY_MS);
      } catch (error) {
        setStatus("error");
    ...
    <main className="mx-auto flex min-h-dvh w-full max-w-6xl flex-col border-edge px-5 pt-6 pb-10 text-center md:border-x">
      ...
      >
        {status === "idle" && (
          ...
          >
            ...
            <p className="mb-5 text-body">or choose from your device</p>
            ...
            <p className="mt-5 text-[13px] text-body">Maximum file size: 10 MB</p>
          </div>
        )}
        ...
        {selection && status !== "idle" && (
          <div className="w-full max-w-md text-center" aria-live="polite">
            ...
            {status === "zip-choice" && (
              <div>
                <p className="mb-4">How do you want to upload this ZIP?</p>
                <div className="flex flex-wrap justify-center gap-3">
                  <Button variant="primary" size="lg" onClick={() => void startUpload("zip")}>
                    Upload ZIP
                  </Button>
                  <Button size="lg" onClick={() => void startUpload("zip-extract")}>
                    Extract &amp; Browse
                  </Button>
                </div>
              </div>
            ...
            {status === "ready" && modeForReady && modeForReady !== "zip-choice" && (
              <div className="flex flex-col items-center gap-3">
                <Button
                  variant="primary"
                  size="lg"
                  onClick={() => void startUpload(modeForReady)}
                >
                  Upload
                </Button>
            ...
            {status === "success" && (
              <div className="text-heading" role="status">
                ...
                <p>Upload complete.</p>
                <p>
                  <a className="break-all text-brand underline" href={resultUrl}>
                    {resultUrl}
                  </a>
                </p>
                <p className="mt-5 text-[13px] text-body">Redirecting…</p>
              </div>
            )}
      ...
      <input
        ...
        onChange={(event) => {
          ...
          if (files?.length) {
            handleSelection({ files: filesFromFileList(files), isFolder: false });
          }
          event.target.value = "";
        }}
      />
      <input
        ...
        onChange={(event) => {
          const { files } = event.target;
          if (files?.length) {
            handleSelection({ files: filesFromDirectoryInput(files), isFolder: true });
          }
          event.target.value = "";
        }}
      />
