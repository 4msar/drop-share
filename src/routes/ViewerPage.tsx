...
export default function ViewerPage() {
  ...
  useEffect(() => {
    ...
    fetchArtifactListing(id, routePath).then(
      (next) => {
        ...
        setSelected(preview ? { file: preview, showSource: false } : null);
      },
      (error: unknown) => {
        if (cancelled) return;
        setListing(null);
        setLoadError(
          error instanceof ArtifactNotFoundError || error instanceof Error
            ? error.message
  ...
  useEffect(() => {
    document.title =
      routePath === "" ? `${id} · Drop Share` : `${id}/${routePath} · Drop Share`;
  }, [id, routePath]);
  ...
  const onDelete = useCallback(async () => {
    if (!window.confirm("Delete this artifact permanently? This cannot be undone.")) {
      return;
    }
    try {
      await deleteArtifact(id);
      setDeleted(true);
      window.setTimeout(() => void navigate("/"), DELETED_REDIRECT_DELAY_MS);
    } catch (error) {
      setActionError(error instanceof Error ? error.message : "Failed to delete artifact.");
    }
  }, [id, navigate]);
  ...
  if (deleted) {
    return (
      <main role="status" className="grid min-h-dvh place-items-center p-6">
        <section className="w-full max-w-md rounded-3xl border border-edge p-8 text-center shadow-lg">
          ...
          <h1 className="mb-2 text-2xl font-medium text-heading">Artifact deleted</h1>
          <p className="text-body">
            The artifact was permanently deleted. You'll be redirected home shortly.
          </p>
        </section>
      </main>
    );
    ...
    return (
      <main className="grid min-h-dvh place-items-center p-6">
        <section role="alert" className="max-w-md text-center">
          <h1 className="mb-2 text-2xl font-medium text-heading">Artifact unavailable</h1>
    ...
    <div className="flex h-dvh flex-col">
      <header className="flex flex-wrap items-start justify-between gap-4 border-b border-edge px-6 py-4">
        <div className="min-w-0">
          <h1 className="mb-0.5 break-all text-lg font-medium text-heading">{title}</h1>
      ...
      {actionError !== null && (
        <p role="alert" className="border-b border-edge px-6 py-2 text-sm text-red-500">
          {actionError}
        </p>
      )}
      ...
      <div className="grid min-h-0 flex-1 md:grid-cols-[280px_1fr] max-md:grid-rows-[auto_1fr]">
        <FileList
          ...
          onPreview={(file) => setSelected({ file, showSource: false })}
        />
        <PreviewPane
          src={previewSrc}
          ...
          onToggleSource={() =>
            setSelected((current) =>
              current === null ? null : { ...current, showSource: !current.showSource },
            )
          }
        />
      </div>
