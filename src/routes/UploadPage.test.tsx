...
describe("selection", () => {
    it("starts on the drop zone", () => {
        ...
        expect(
            screen.getByRole("heading", {
                name: /drop files, zips, or folders here/i,
            }),
        ).toBeTruthy();
    });
    ...
    it("asks how to handle a lone ZIP rather than guessing", () => {
        ...
        expect(
            screen.getByText(/how do you want to upload this zip/i),
        ).toBeTruthy();
        expect(screen.getByRole("button", { name: "Upload ZIP" })).toBeTruthy();
        expect(
            screen.getByRole("button", { name: /extract & browse/i }),
        ).toBeTruthy();
    });

    it("goes straight to a single Upload action for ordinary files", () => {
        renderPage();
        choose("Choose files to upload", [
            fakeFile("a.txt"),
            fakeFile("b.txt"),
        ]);

        expect(screen.getByRole("button", { name: "Upload" })).toBeTruthy();
        expect(screen.getByText(/files selected/i).textContent).toMatch(
            /^2 files selected/,
        );
    });
    ...
    it("reports the size cap client-side and does not attempt the upload", () => {
        renderPage();
        choose("Choose files to upload", [
            fakeFile("huge.bin", 11 * 1024 * 1024),
        ]);

        expect(screen.getByRole("alert").textContent).toMatch(
            /exceeding the 10 MB maximum file size/i,
        );
...
describe("uploading", () => {
    ...
    it("sends the chosen ZIP mode through", async () => {
        ...
        await waitFor(() => expect(uploadArtifact).toHaveBeenCalledOnce());
        expect(uploadArtifact.mock.calls[0][0] as UploadMode).toBe(
            "zip-extract",
        );
    });
    ...
    it("surfaces a rejected upload instead of appearing to succeed", async () => {
        uploadArtifact.mockRejectedValue(
            new Error("Upload exceeds the maximum artifact size"),
        );
        renderPage();
        choose("Choose files to upload", [fakeFile("a.txt")]);
        fireEvent.click(screen.getByRole("button", { name: "Upload" }));
    ...
    it("reports progress while the upload runs", async () => {
        ...
        await waitFor(() =>
            expect(screen.getByText(/uploading… 42%/i)).toBeTruthy(),
        );
    });
});
...
describe("recent artifacts", () => {
    it("shows no recent-artifacts toggle when nothing has been viewed yet", () => {
        renderPage();
        expect(
            screen.queryByRole("button", { name: /recent artifacts/i }),
        ).toBeNull();
    });
    ...
    it("closes the drawer when clicking the backdrop behind it", () => {
        ...
        expect(screen.getByLabelText("Close recent artifacts")).toBeTruthy();

        fireEvent.click(screen.getByLabelText("Close recent artifacts"));

        expect(screen.getByLabelText("Close recent artifacts")).toHaveClass(
            "pointer-events-none",
        );

        expect(
            screen
                .getByRole("button", { name: /^recent artifacts$/i })
