...
ZIP, or a folder in the browser (or via the `artifact` CLI) and get back an
...
this with `artifact upload some.zip` (as-is) vs `artifact upload some.zip
--extract`.
...
├── cli/                       standalone `artifact` CLI (separate npm package)
│   └── src/index.ts
